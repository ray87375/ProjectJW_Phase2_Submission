// Theme switcher - allows users to toggle between dark and light mode
(function(){
...
async function loadVideos() {
  /**
   * Fetch videos from the API and display them on the page.
   * Each video gets its own card with a title and video player.
   */
  try {
    ...
    if (!Array.isArray(data) || data.length === 0) {
      videoListEl.innerHTML = '<p class="text-gray-500">No videos yet.</p>'; 
      return;
    }
    
    videoListEl.innerHTML = '';
    data.forEach(v => {
      // Create a container for each video with title and player
      ...
      // Set max width to video's natural resolution to prevent upscaling
...
document.getElementById('contactForm').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    if (res.ok) {
      out.textContent = 'Thanks! I will get back to you shortly.'; 
      out.className = 'text-sm mt-2 text-green-600'; 
      form.reset();
    } else {
      const err = await res.json().catch(()=>({})); 
      out.textContent = 'Error: ' + (err.error || 'could not send'); 
      out.className = 'text-sm mt-2 text-red-600';
    }
  } catch (err) { 
    console.error(err); 
...
async function showLocation() {
  /**
   * Display the gym location using Google Maps embed.
   * Shows a loading state while the map loads.
   */
